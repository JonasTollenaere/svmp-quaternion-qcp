...
int main() {
    ...
    csvFile.open("QuaternionSCIPInverseResults.csv");

    // Write the header
    csvFile << "Convex hull?,Break symmetry?,Container,Item,Time(s),Scale,Gap,Explored,Unexplored,TranslationX,TranslationY,TranslationZ,RotationW,RotationX,RotationY,RotationZ" << std::endl;
    csvFile.flush();
...
//            SCIP_CALL(SCIPsetIntParam(scip, "parallel/maxnthreads", 20) );
                        ...
                        SCIP_Real SValue = 1.0/inverseSValue;
                        SCIP_Real TxValue = SCIPgetSolVal(scip, SCIPgetBestSol(scip), Tx)/inverseSValue;
                        SCIP_Real TyValue = SCIPgetSolVal(scip, SCIPgetBestSol(scip), Ty)/inverseSValue;
                        SCIP_Real TzValue = SCIPgetSolVal(scip, SCIPgetBestSol(scip), Tz)/inverseSValue;
                        ...
                        auto runTime = SCIPgetSolvingTime(scip);
                        auto gap = SCIPgetGap(scip);
                        auto explored = SCIPgetNNodes(scip);

                        // Write the results to the .csv file
                        csvFile << useConvexHull << "," << breakSymmetry << "," << containerModelSpaceMesh->getName() << "," << itemModelMesh->getName() << "," << runTime << "," << SValue << "," << gap << "," << explored << "," << explored << "," << TxValue << "," << TyValue << "," << TzValue << "," << QwValue << "," << QxValue << "," << QyValue << "," << QzValue << std::endl;
